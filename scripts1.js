const commonQueries = {
    ...
    "Filing": [
      "How to file a shipping bill?",
      "What are the steps to file a bill of entry?",
      "How to amend a filed bill?"
    ],
    'ePayment': [
  ...
  function handleUserInput(input) {
    ...
    if (commonQueries[input]) {
      ...
      setTimeout(() => {
        addMessage(answers[input], 'assistant', true); // Indicating this message contains HTML
      }, 500);
  ...
  function addMessage(content, sender, isHTML = false) {
    ...
    if (isHTML) {
      message.innerHTML = content; // Use innerHTML for formatted content
    } else {
      message.textContent = content; // Use textContent for plain text
    }
  ...
  function resetChat() {
    ...
    chatContent.innerHTML = `
      <div class="message assistant">
        <section>
          <img src="assisstant-image.jpg" alt="Assistant Image" />
        </section>
        <section>
          Hi, I'm Vaani, your Virtual Assistant. You can select from the options below or you can type your question in the space provided:
        </section>
      </div>
  ...
  function sendToRasa(message) {
    fetch('http://localhost:5005/webhooks/rest/webhook', {
      ...
      .then(data => {
        const responses = data.map(item => item.text);
        responses.forEach(response => addMessage(response, 'assistant'));
      })
      .catch(error => {
  ...
  function startRecognition() {
    ...
    recognition.onerror = (event) => {
      ...
      addMessage('Speech recognition error detected: ' + event.error, 'assistant');
    };
